...
def _log_chemprop_epoch_curves(split_out: Path, split_idx: int) -> None:
    """Read Chemprop's metrics.csv and log per-epoch train/val loss to W&B."""
    ...
    for _, row in df.iterrows():
        ...
        for col in df.columns:
            ...
            val = row[col]
            if pd.notna(val):
                log[f"split_{split_idx}/{col}"] = float(val)
        if log:
            wandb.log(log, step=step + split_idx * 1000)
...
def train_cv(
    ...
    for i, split in enumerate(splits):
        ...
        # Log per-split summary to W&B
        wandb.log({
            f"split_{i}/mae": m["mae"],
            f"split_{i}/rae": m["rae"],
            f"split_{i}/r2": m["r2"],
            f"split_{i}/spearman": m["spearman"],
        })
